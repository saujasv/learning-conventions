...
class DataCollatorForPreference:
    ...
    def __call__(self, examples):
        ...
        rejected_input_ids = [
            self.processor.tokenizer(x["rejected"], add_special_tokens=False)[
                "input_ids"
            ]
            for x in processed_inputs
        ]
        ...
        chosen_input_ids = [
            x + [self.processor.tokenizer.eos_token_id] for x in chosen_input_ids
        ]
        rejected_input_ids = [
        ...
        output = self.torch_call(
            [
                {
                    "prompt_input_ids": prompt_features["input_ids"][0],
                    "pixel_values": prompt_features["pixel_values"],
                    "chosen_input_ids": chosen_input_ids,
                    "rejected_input_ids": rejected_input_ids,
                }
                for prompt_features, chosen_input_ids, rejected_input_ids in zip(
                    processed_features, chosen_input_ids, rejected_input_ids
                )
            ]
        )
...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if data_collator is None:
            data_collator = DataCollatorForPreference(
                processor=processing_class, pad_token_id=self.padding_value
            )
    ...
    def process_row(
        ...
        processed_features = processor(
            images=[Image.open(img) for img in features["images"]],
