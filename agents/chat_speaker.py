...
class ChatSpeaker(ChatAgent):
    ...
    def format_trial(
        ...
        target_prompt = [
            {
                "type": "text",
                "text": f"the target image is {self.get_label(context, trial.target)}. Describe the target image to the listener. Generate only the message containing the description.",
            }
        ]
